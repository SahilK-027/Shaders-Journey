...
class ShaderRenderer {
  ...
  initGeometry() {
    ...
    const texture1Img = new THREE.TextureLoader().load(texture1);
    ...
    this.material = new THREE.ShaderMaterial({
      ...
      uniforms: {
        ...
        uTime: { value: 0 }, // Time uniform for animation
      },
      side: THREE.DoubleSide, // Render both sides of the plane
    });

    this.mesh = new THREE.Mesh(this.geometry, this.material);
    this.mesh.scale.y = 2 / 4.6;
  ...
  animate() {
    this.material.uniforms.uTime.value = performance.now() / 1000; // Update time uniform
